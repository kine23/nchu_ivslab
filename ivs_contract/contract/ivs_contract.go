...
import (
	...
	"fabriclab.com/mylab_ivs/ivs_contract/tools"
)
...
func (o *ProjectContract) Exists(ctx contractapi.TransactionContextInterface, index string) (bool, error) {
	...
	if !isAllowed {
		return false, fmt.Errorf("only members of the allowed organizations with the admin role can check if a project exists")
	}
...
func (o *ProjectContract) Insert(ctx contractapi.TransactionContextInterface, pJSON string) error {
	...
	if !isAllowed {
		return fmt.Errorf("only members of the allowed organizations with the admin role can check if a project exists")
	}
...
func (o *ProjectContract) SelectByIndex(ctx contractapi.TransactionContextInterface, pJSON string) (*model.Project, error) {
	...
	if !isAllowed {
		return nil, fmt.Errorf("only members of the allowed organizations with the admin role can check if a project exists")
	}
